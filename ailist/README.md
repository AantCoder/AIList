...
|---|---|---|---|
| Whisper `turbo` (audio/video transcription) | `~/.cache/huggingface/hub` | First audio or video file attached | ~800 MB |
| Piper voice model (TTS) | `~/.cache/ailist/piper` | First `piper_synthesize` call | ~50–100 MB per voice |
| Qdrant embedding model | `~/.cache/ailist/fastembed` | First `await ai.mcp_connect("qdrant")` | ~90 MB |
| Serena language servers (LSP) | managed by uv | First symbol operation in a new language | varies |

> All models are downloaded once and shared across all projects on the machine. Override the Piper/Qdrant cache location with the `AILIST_CACHE_DIR` environment variable or by setting `ai._cache_dir = Path("/my/cache")` before first use.